...
func TestReplica_EnforceRetention_SkipsCurrentGeneration(t *testing.T) {
	...
	// Test that the active generation snapshot is still present, even though it is expired.
	snapshots, err := r.Snapshots(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if len(snapshots) != 1 {
		t.Fatalf("len(snapshots)=%d, want %d; snapshots=%v", len(snapshots), 2, snapshots)
	}
	if snapshots[0].Generation != "bbbb000000000001" || snapshots[0].Index != 0 {
		t.Fatalf("snapshot[0]=%v, want generation=bbbb000000000001 index=0", snapshots[0])
	}
}

// TestReplica_EnforceRetention_RemoveCurrentGenerationExpiredSnapshot test currently asserts that
// old snapshot within the active generation still get deleted.
func TestReplica_EnforceRetention_RemoveCurrentGenerationExpiredSnapshot(t *testing.T) {
	const retention = 1 * time.Minute
	...
	mustWriteStubSnapshot(t, c, "aaaa000000000001", 0, expired)
	mustWriteStubSnapshot(t, c, "bbbb000000000001", 0, recent)
	mustWriteStubSnapshot(t, c, activeGen, 0, expired)
	mustWriteStubSnapshot(t, c, activeGen, 5, recent)
	...
	// Test removes the expired snapshot within the active generation, so we should have 2 snapshots left.
	if len(snapshots) != 2 {
		t.Fatalf("len(snapshots)=%d, want %d; snapshots=%v", len(snapshots), 2, snapshots)
	}
	if !slices.ContainsFunc(snapshots, func(s litestream.SnapshotInfo) bool { return s.Generation == activeGen && s.Index == 5 }) {
		t.Fatalf("missing expected snapshot generation=%s index=5; got %v", activeGen, snapshots)
	}
	if !slices.ContainsFunc(snapshots, func(s litestream.SnapshotInfo) bool { return s.Generation == "bbbb000000000001" && s.Index == 0 }) {
		t.Fatalf("missing expected snapshot generation=bbbb000000000001 index=0; got %v", snapshots)
	}
}
